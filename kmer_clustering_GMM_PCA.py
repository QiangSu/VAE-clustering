...
def main(args):
    ...
    # 5. Apply GMM clustering on PCA-reduced data
    gmm = GaussianMixture(n_components=args.clusters, random_state=42)
    clusters = gmm.fit_predict(reduced_kmers)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cluster k-mers from a CSV file using PCA, Gaussian Mixture Models, compute silhouette score, and visualize via UMAP."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of GMM clusters to use (default: 10)."
    )
    parser.add_argument(
        "--pca_dim",
        type=int,
