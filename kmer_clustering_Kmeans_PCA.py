...
def main(args):
    ...
    # 5. Apply K-Means clustering on PCA-reduced data
    kmeans = KMeans(n_clusters=args.clusters, random_state=42, n_init='auto')
    clusters = kmeans.fit_predict(reduced_kmers)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cluster k-mers from a CSV file using PCA, K-Means, compute silhouette score, and visualize via UMAP."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of K-Means clusters to use (default: 10)."
    )
    parser.add_argument(
        "--pca_dim",
        type=int,
